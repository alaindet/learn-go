...
func TestHoursInRadians(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(testName(c.input), func(t *testing.T) {
			result := hoursInRadians(c.input)
			if !circaEqualFloats(result, c.expected, 1e-7) {
				t.Fatalf("Wanted %v radians, but got %v", c.expected, result)
			}
		})
	}
...
func TestHourHandPoint(t *testing.T) {
	...
	for _, c := range cases {
		t.Run(testName(c.time), func(t *testing.T) {
			result := hourHandPoint(c.time)
			if !circaEqualPoints(result, c.point) {
				t.Fatalf("Wanted %v Point, but got %v", c.point, result)
			}
		})
	}
}
